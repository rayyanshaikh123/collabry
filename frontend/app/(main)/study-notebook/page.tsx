...
export default function StudyNotebooksPage() {
  ...
  if (isLoading) {
    return (
      <div className="h-full w-full flex items-center justify-center box-border">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto"></div>
          <p className="mt-4 text-slate-600 font-semibold">Loading notebooks...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-full w-full flex items-center justify-center box-border">
        <div className="text-center">
          ...
          <p className="text-red-600 font-semibold">Failed to load notebooks</p>
  ...
  return (
    <div className="h-full flex flex-col relative bg-slate-50 overflow-hidden -m-4 md:-m-8">
      {/* Header */}
      <div className="flex-shrink-0 bg-white border-b-4 border-slate-100 px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-black text-slate-800">Study Notebooks 📓</h1>
      ...
      <div className="flex-1 overflow-y-auto p-8">
            ...
            {notebooks.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <Card className="flex items-center gap-4 bg-gradient-to-br from-indigo-50 to-white border-indigo-100">
                  ...
                  <div>
                    <p className="text-3xl font-black text-slate-800">{notebooks.length}</p>
                    <p className="text-xs text-indigo-600 font-bold uppercase tracking-wider">Total Notebooks</p>
                  </div>
                </Card>
                <Card className="flex items-center gap-4 bg-gradient-to-br from-emerald-50 to-white border-emerald-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-600 font-bold uppercase tracking-wider">Total Sources</p>
                  </div>
                </Card>
                <Card className="flex items-center gap-4 bg-gradient-to-br from-amber-50 to-white border-amber-100">
                  <div className="w-14 h-14 bg-amber-400 rounded-2xl flex items-center justify-center text-slate-800 shadow-lg shadow-amber-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-600 font-bold uppercase tracking-wider">Total Artifacts</p>
                  </div>
                </Card>
              </div>
            )}
            ...
            {notebooks.length === 0 && (
              <Card className="text-center py-20">
                <div className="mx-auto w-32 h-32 bg-indigo-500 rounded-[2rem] flex items-center justify-center mb-6 shadow-xl shadow-indigo-200">
                    ...
                    <div onClick={() => handleOpenNotebook(notebook._id)}>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <button
                          ...
                          className="p-2 hover:bg-rose-50 rounded-xl transition-all hover:scale-110"
                        >
                          <ICONS.trash className="w-5 h-5 text-rose-500" strokeWidth={2.5} />
                        </button>
                      </div>

                      {/* Stats */}
                      <div className="flex items-center gap-2 mb-4">
                        <div className="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-xl text-xs font-bold">
                          <ICONS.fileText className="w-3.5 h-3.5" strokeWidth={2.5} />
                          <span>{notebook.sources?.length || 0} sources</span>
                        </div>
                        <div className="flex items-center gap-1 px-3 py-1.5 bg-amber-100 text-amber-700 rounded-xl text-xs font-bold">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t-2 border-slate-100">
                        ...
                        <div className="flex items-center gap-1 text-indigo-500 font-bold text-sm group-hover:translate-x-1 transition-transform">
                          <span>Open</span>
