...
class MongoMemoryStore:
    ...
    # -----------------------------
    # Voice Tutor Session Storage
    # -----------------------------
    def save_voice_session(self, session_data: Dict[str, Any]) -> bool:
        """
        Save voice tutor session to MongoDB.
        ...
        try:
            collection = self._db["voice_tutor_sessions"]
            ...
            logger.info(f"✓ Saved voice tutor session: {session_data['session_id']}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to save voice tutor session: {e}")
            return False
    
    def load_voice_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        """
        Load voice tutor session from MongoDB.
        ...
        try:
            ...
            if session:
                session.pop("_id", None)  # Remove MongoDB ObjectId
                logger.info(f"✓ Loaded voice tutor session: {session_id}")
            
            return session
            
        except Exception as e:
            logger.error(f"Failed to load voice tutor session: {e}")
            return None
    
    def save_student_profile(self, profile_data: Dict[str, Any]) -> bool:
        """
        Save student profile to MongoDB.
        
        Args:
            profile_data: Dictionary with student profile information
        ...
        try:
            collection = self._db["student_profiles"]
            
            # Upsert profile by user_id + notebook_id
            result = collection.replace_one(
                {
                    "user_id": profile_data["user_id"],
                    "notebook_id": profile_data["notebook_id"]
                },
                profile_data,
                upsert=True
            )
            
            logger.info(f"✓ Saved student profile: {profile_data['user_id']}/{profile_data['notebook_id']}")
            return True
    ...
    def load_student_profile(self, user_id: str, notebook_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if profile:
                ...
                logger.info(f"✓ Loaded student profile: {user_id}/{notebook_id}")
